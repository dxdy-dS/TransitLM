...
export async function GET(request: Request) {
  ...
  try {
    const location = q
      ? `${BASE_URL}/onecall?q=${encodeURIComponent(q)}&appid=${API_KEY}&units=${units}&exclude=${exclude}`
      : `${BASE_URL}/onecall?lat=${lat}&lon=${lon}&appid=${API_KEY}&units=${units}&exclude=${exclude}`;
